...
namespace OB.DAL
{
    public class DatabaseHelper
    {
        ...
        public static OiiOBookDCDataContext GetDataModelDataContext()
        {
            try
            {
                _connectionString = ConfigurationManager.ConnectionStrings["DBConnectionString"].ToString();
                var db = new OiiOBookDCDataContext(_connectionString);
                return db;
            }
            catch (Exception ex)
            {
        ...
        public static void Update<T>(OiiOBookDCDataContext db, T obj) where T : class
        {
        ...
        public static void Update<T>(OiiOBookDCDataContext db, T objSrc, T objDest) where T : class
        {
            //using (var db = GetDashboardData())
            //{   
            //    db.GetTable<T>().Attach(obj);
        ...
        public static void Update<T>(OiiOBookDCDataContext db, T obj, long currentuser, Action<T> update) where T : class
        {
            //using (var db = GetDashboardData())
            //{   
            //    db.GetTable<T>().Attach(obj);
        ...
        public static void UpdateAll<T>(OiiOBookDCDataContext db, List<T> items, Action<T> update) where T : class
        {
            //using (var db = GetDashboardData())
            //{
        ...
        public static void UpdateAll<T>(OiiOBookDCDataContext db, List<T> items, long currentuser, Action<T> update) where T : class
        {
            //using (var db = GetDashboardData())
            //{
        ...
        public static void Delete<T>(OiiOBookDCDataContext db, T entity) where T : class, new()
        {
            //using (var db = GetDashboardData())
            //{
            //    Table<T> table = db.GetTable<T>();
            ...
            foreach (PropertyInfo pInfo in propertyInfos)
            {
                ...
                if (isInserting == true)
                {
                    switch (pInfo.Name)
                    {
                        ...
                        case "CreatedDate":
                            {
                                //OB.DAL.Basic.GlobalDA.GetServerDateTime()
                                pInfo.SetValue(obj, OB.DAL.Basic.GlobalDA.GetServerDateTime(), null);
                                break;
                            }
                        case "IsRemoved":
                            {
