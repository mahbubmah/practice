...
namespace DAL
{
    public class DatabaseHelper
    {
        ...
        public static OiiOMartDBDataContext GetDataModelDataContext()
        {
            try
            {
                _connectionString = ConfigurationManager.ConnectionStrings["DBConnectionString"].ToString();
                var db = new OiiOMartDBDataContext(_connectionString);
                return db;
            }
            catch (Exception ex)
            {
        ...
        public static void Update<T>(OiiOMartDBDataContext db, T obj) where T : class
        {
        ...
        public static void Update<T>(OiiOMartDBDataContext db, T objSrc, T objDest) where T : class
        {
            //using (var db = GetDashboardData())
            //{   
            //    db.GetTable<T>().Attach(obj);
        ...
        public static void Update<T>(OiiOMartDBDataContext db, T obj, long currentuser, Action<T> update) where T : class
        {
            //using (var db = GetDashboardData())
            //{   
            //    db.GetTable<T>().Attach(obj);
        ...
        public static void UpdateAll<T>(OiiOMartDBDataContext db, List<T> items, Action<T> update) where T : class
        {
            //using (var db = GetDashboardData())
            //{
        ...
        public static void UpdateAll<T>(OiiOMartDBDataContext db, List<T> items, long currentuser, Action<T> update) where T : class
        {
            //using (var db = GetDashboardData())
            //{
        ...
        public static void Delete<T>(OiiOMartDBDataContext db, T entity) where T : class, new()
        {
            //using (var db = GetDashboardData())
            //{
            //    Table<T> table = db.GetTable<T>();
                        ...
                        case "CreatedDate":
                            {
                                ...
                                pInfo.SetValue(obj, DAL.Basic.GlobalDA.GetServerDateTime(), null);
                                break;
                            }
                        case "IsRemoved":
                            {
